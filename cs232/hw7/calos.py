...
#   added calls to set MMU registers after CPU's registers have been set
############################################################################
...
class CalOS:
    ...
    def timer_isr(self, cpu):
        ...
        if len(self._ready_q) == 0:
            ...
            # set mmu registers so that reloc = low_mem and limit = high_mem - low_mem
            cpu.set_mmu_registers(self._current_proc[cpu.get_num()].get_low_mem(), 
                                  self._current_proc[cpu.get_num()].get_high_mem() - self._current_proc[cpu.get_num()].get_low_mem())
            return
