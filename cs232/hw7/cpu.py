...
#   added MMU related variables and methods
############################################################################
...
class CPU:
    ...
    def __init__(self, ram, os, num=0):
        ...
        self._ram = ram
        self._mmu = ram.MMU(ram)                # Create and store an MMU object, passing in ram as parameter
        self._os = os
        self._debug = False
        ...
        while True:
            ...
            if self._debug:
                ...
                print("CPU {}: executing code at [{}]: {}".format(self._num, self._registers['pc'],
                                                          self._mmu.__getval__(self._registers['pc'])))

            # Execute the next instruction.
            self.parse_instruction(self._mmu.__getval__(self._registers['pc']))
    ...
    def _get_value_at(self, addr):
        ...
        or hex.'''
        addr = eval(addr[1:])
        return self._mmu.__getval__(addr)
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.__storeval__(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.__storeval__(eval(dst), srcval)
    ...
    def handle_add(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.__storeval__(self._registers[dst[1:]], self._mmu.__getval__(self._registers[dst[1:]]) + srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.__storeval__(eval(dst), self._mmu.__getval__(eval(dst)) + srcval)
