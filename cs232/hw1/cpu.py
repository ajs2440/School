...
@author Advait Scaria
@date 02/10/20
    Added:
        1. More stuff in the run() method to run the appropriate chunk
            depending on whether it is in batch mode or not
'''
...
class CPU(threading.Thread):
    ...
    # runs _run_program() if not in _batch_mode
    # otherwise runs _run_program() continuously until startAddr == 0,
        #   clears register and sets pc after each program ends execution
    def run(self):
        '''Overrides run() in thread.  Called by calling start().'''

        # run normally if not in batch_mode
        if not self._batch_mode:
            self._run_program()
        # else run batches until start address is 0
        else:
            while (self._ram[self._startAddr] != 0):    # checks if start address is 0
                self.set_pc(self._ram[self._startAddr]) # set pc to start address
                self._run_program()                     # run program
                self._registers['reg0'] = 0             # clear registers
                self._registers['reg1'] = 0
                self._registers['reg2'] = 0
                self._startAddr += 1                    # increment start address to run next instruction
