...
@dat 02/10/20
    Added:
        1. an 'R' instruction that takes the starting address of a sequence of
            programs to run those programs sequentially.
        2. a _run_batch() function that starts a new thread and sets batch_mode to True
            to run the batches
...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: runs programs sequentially starting at location <addr>")     # new R instruction for homework
                    continue
                ...
                elif instr.upper().startswith('R '):   # call _run_batch() if instruction starts with R
    ...
    # _run_batch() function to run batches of programs by setting
    # batch_mode to true
